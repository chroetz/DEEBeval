...
showSkillScoreTable <- function(d, caption) {
  nMethods <- length(unique(d$method))
  dText <- 
    d |> 
    mutate(text = sprintf("%.3f (%d)", value, rank)) 
  dWide <- 
    d |> 
...
showRankScoreTable <- function(d, caption) {
  d |> 
    kableExtra::kbl(
      caption = caption,
      booktabs = TRUE,
  ...
  for (lss in unique(taskData$loss)) {
    ...
    for (onr in unique(lssData$obsNr)) {
      ...
      cat(showSkillScoreTable(
        obsData,
        paste0("Task ", tnr, ", obsNr ", onr, ": ", lss, "-SkillScores (rank)")))
